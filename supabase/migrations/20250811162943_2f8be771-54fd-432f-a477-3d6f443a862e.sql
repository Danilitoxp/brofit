-- Security hardening migration (idempotent where possible)
...
-- 6) Storage policies (create if missing) using dynamic SQL wrappers
-- Avatars: public read
DO $$ BEGIN
  IF NOT EXISTS (
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    EXECUTE $$CREATE POLICY "Users can upload own avatar" ON storage.objects FOR INSERT TO authenticated WITH CHECK (
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    EXECUTE $$CREATE POLICY "Users can update own avatar" ON storage.objects FOR UPDATE TO authenticated USING (
      bucket_id = 'avatars' AND auth.uid()::text = (storage.foldername(name))[1]
    ) WITH CHECK (
      bucket_id = 'avatars' AND auth.uid()::text = (storage.foldername(name))[1]
    )$$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    EXECUTE $$CREATE POLICY "Users can delete own avatar" ON storage.objects FOR DELETE TO authenticated USING (
      bucket_id = 'avatars' AND auth.uid()::text = (storage.foldername(name))[1]
    )$$;
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    EXECUTE $$CREATE POLICY "Admins can manage exercise images" ON storage.objects FOR ALL TO authenticated USING (
